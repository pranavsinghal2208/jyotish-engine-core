"""Auto-seed numerology profiles on first startup (runs only if table is empty)."""
...
PEOPLE = [
    ...
    ("Male",   "elon_x",      "X",          "",        "Ae A-12 Musk",    "04-05-2020"),
    ...
    ("Female", "celine_d",    "Celine",     "Marie",   "Dion",            "30-03-1968"),
...
def seed_numerology_profiles_if_empty(db):
    """Insert all profiles if the table is currently empty. Safe to call on every startup."""
    count = db.query(NumerologyProfile).count()
    if count > 0:
        return
    ...
    db.commit()
    print(f"[seed] Inserted {len(PEOPLE)} numerology profiles.")
